...
describe('stock count repository', () => {
  ...
  it('creates stock count session with active variants snapshot', () => {
    ...
    const itemA = details.items.find(
      (item: StockCountItemTestRow) => item.barcode === 'COUNT-A',
    )
  ...
  it('rejects approving session with uncounted items', () => {
    ...
    expect(
      getVariantStock(getVariantByBarcode('UNCOUNTED001')!.variant_id),
    ).toBe(10)
  })
  ...
  it('adds products created during an open stock count session', () => {
    ...
    seedStockCountProduct({
      name: 'Product Added During Count',

      barcode: 'LIVE-COUNT-001',
    ...
    const addedItem = details.items.find(
      (item: StockCountItemTestRow) => item.barcode === 'LIVE-COUNT-001',
    )

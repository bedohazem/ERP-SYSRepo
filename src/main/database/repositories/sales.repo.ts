...
export type UpdateClosedShiftCashSaleCorrectionInput = {
...
export function createClosedShiftCashSaleCorrection(
  ...
  const pricedItems = preparedItems.map((item) => {
    ...
    const unitPrice = roundMoney(Number(variant?.sell_price || 0))

    if (!Number.isFinite(unitPrice) || unitPrice <= 0) {
      throw new Error(
        `سعر بيع الصنف ${variant?.product_name || item.variant_id} غير صحيح`,
      )
    }
  ...
  const tx = db.transaction(() => {
    reopenCashShiftVarianceForCorrection({
      ...
      reason: 'إضافة فاتورة بيع تصحيحية على فرق تمت مراجعته',
    })
    const saleResult = db
      .prepare(
        `
...
export function updateClosedShiftCashSaleCorrection(
  input: UpdateClosedShiftCashSaleCorrectionInput,
) {
  const db = getDb()
  ...
  if (!correction) {
    throw new Error('فاتورة تصحيح فرق الشفت غير موجودة')
  }
  ...
  if (!saleId) {
    throw new Error('فاتورة التصحيح المرتبطة غير صحيحة')
  }
  ...
  if (!sale || sale.cancelled_at) {
    throw new Error('فاتورة التصحيح غير موجودة أو ملغاة')
  }

  const dependentReturns = db
    .prepare(
      `
      SELECT
        COUNT(*) AS count
  ...
  if (Number(dependentReturns.count || 0) > 0) {
    throw new Error('لا يمكن تعديل فاتورة التصحيح بعد وجود مرتجع عليها')
  }

  const dependentExchanges = db
    .prepare(
      `
      SELECT
        COUNT(*) AS count
  ...
  if (Number(dependentExchanges.count || 0) > 0) {
    throw new Error('لا يمكن تعديل فاتورة التصحيح بعد وجود استبدال عليها')
  }
  ...
  if (oldItems.length === 0) {
    throw new Error('لا توجد أصناف داخل فاتورة التصحيح')
  }

  const quantityMap = new Map<number, number>()

  for (const rawItem of input.items) {
    const variantId = Number(rawItem.variant_id || 0)
    ...
    quantityMap.set(
      ...
      Number(quantityMap.get(variantId) || 0) + quantity,
    )
  }
  ...
  for (const oldItem of oldItems) {
    const variantId = Number(oldItem.variant_id)

    oldQuantityMap.set(
      variantId,

      Number(oldQuantityMap.get(variantId) || 0) +
  ...
  for (const [variantId, quantity] of quantityMap) {
    ...
    if (!variant) {
      throw new Error(`الصنف رقم ${variantId} غير موجود أو غير مفعل`)
    }

    const currentStock = Number(variant.stock || 0)

    const restorableOldQty = Number(oldQuantityMap.get(variantId) || 0)

    const availableForEdit = currentStock + restorableOldQty

    if (quantity > availableForEdit) {
      throw new Error(
        `المخزون غير كافي للصنف ${variant.product_name}. المتاح بعد إلغاء أثر الفاتورة القديمة: ${availableForEdit}`,
      )
    }
    ...
    if (unitPrice <= 0) {
      throw new Error(`سعر بيع الصنف ${variant.product_name} غير صحيح`)
    }
  ...
  if (grandTotal <= 0) {
    throw new Error('إجمالي فاتورة التصحيح غير صحيح')
  }

  const businessDate = getShiftBusinessDate(Number(correction.shift_id))
  ...
  const tx = db.transaction(() => {
    reopenCashShiftVarianceForCorrection({
      ...
      reason: `تعديل فاتورة التصحيح #${saleId}`,
    })

    const restoreOldStock = db.prepare(
      `
          INSERT INTO stock_movements (
            variant_id,
            type,
          ...
          VALUES (
            ...
            ?,
            'shift_variance_sale_correction_edit_reverse',
    ...
    for (const oldItem of oldItems) {
      restoreOldStock.run(
        Number(oldItem.variant_id),

        Number(oldItem.quantity || 0),

        saleId,

        `عكس مخزون فاتورة تصحيح #${saleId} قبل تعديلها`,
      )
    }
    ...
    db.prepare(
      ...
      [`فاتورة تصحيح فرق الشفت #${correction.shift_id}`, notes || null]
        .filter(Boolean)
        .join(' — '),
    ...
    const removeNewStock = db.prepare(
      `
          INSERT INTO stock_movements (
            variant_id,
            type,
          ...
          VALUES (
            ...
            ?,
            'shift_variance_sale_correction_edit',
    ...
    for (const item of preparedItems) {
      ...
      insertItem.run(
        ...
        String(item.variant.product_name || ''),
      ...
      removeNewStock.run(
        ...
        `مخزون فاتورة تصحيح معدلة #${saleId}`,
      )
    }

    const correctionUpdate = db
      .prepare(
        `
    ...
    if (Number(correctionUpdate.changes || 0) !== 1) {
      throw new Error('تعذر تحديث تصحيح فرق الشفت')
    }

    createActivityLog({
      user_id: actorId,

      action: 'shift_variance_sale_correction_updated',
    ...
    return {
      ...
      shift_id: Number(correction.shift_id),

      grand_total: grandTotal,

      review: getCashShiftVarianceReview(Number(correction.variance_id)),
    }
  })
...
export function createSaleReturn(input: {
  ...
  const tx = db.transaction(() => {
     ...
     * لا نسمح بإنشاء حالة سالبة
     * أو إسقاط النقاط بصمت.
     */
